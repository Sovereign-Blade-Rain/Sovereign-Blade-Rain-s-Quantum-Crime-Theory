...
import { AiStage, type Slider, type TerraformConfig, TerraformTarget, ControlConfig, SoundConfig, Modulation, ModulationSource, ModulationTarget, CameraData, ViewMode, ShipConfig, ShipModulation, ShipModulationTarget, PlayerRole, RolePlayState } from '../types';
import { AppContextType } from '../context/AppContext';
import { v4 as uuidv4 } from 'uuid';
import { EDITMODE } from '../config';
import { auth, signInWithGoogle, logout as firebaseLogout, createUserProfile, getUserProfile } from '../firebase';
...
const getPlanet1Distance = (p_vec: number[] | Float32Array, uniforms: any, t: number) => {
    ...
    // Simple hash for height
    const h = (Math.abs(Math.sin(gx * 12.9898 + gz * 78.233) * 43758.5453) % 1) * 60.0 + 10.0;
    
    const localX = ((x % grid) + grid) % grid - grid/2;
    const localZ = ((z % grid) + grid) % grid - grid/2;
    
    // Box distance (buildings grow "down" into positive Y)
    const dx = Math.abs(localX) - 6.0;
    const dy = Math.abs(y - h/2) - h/2; 
    const dz = Math.abs(localZ) - 6.0;
    const dBox = Math.max(dx, dy, dz);

    // Antenna distance
    const adx = Math.abs(localX) - 0.2;
    const ady = Math.abs(y - (h + 2.0)) - 2.0;
    const adz = Math.abs(localZ) - 0.2;
    const dAntenna = Math.max(adx, ady, adz);
    
    const dGround = -y; // Ground at y=0
    
    return Math.min(dBox, dAntenna, dGround);
};
...
import { WorldPreset, MapPreset } from '../types';
...
export const useAppStore = (): Omit<AppContextType, keyof ReturnType<typeof useDummyHandlers>> => {
  ...
  const [rolePlayState, setRolePlayState] = useState<RolePlayState>({
    ...
    isInsideShip: true,
    isLanded: false,
    prisonerStatus: 'imprisoned',
    fuel: 100,
    isDetected: false,
    isAdmin: false,
    isGrandAdmin: false,
    hasMasterPlanAccess: false,
    employeeRank: 100,
  });
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        ...
        const unsubProfile = getUserProfile(currentUser.uid, (profile) => {
          setUserProfile(profile);
          // Sync role with rolePlayState
          if (profile.role === 'admin') {
            setRolePlayState(prev => ({ ...prev, isAdmin: true }));
          }
        });
        return () => unsubProfile();
      } else {
        setUserProfile(null);
        setRolePlayState(prev => ({ ...prev, isAdmin: false }));
      }
    });
    return () => unsubscribe();
  }, []);
  ...
  const loadSession = useCallback(async (sessionId: string) => {
    try {
      ...
      for (const map of MAP_PRESETS) {
        const planet = map.planets.find(p => p.id === sessionId);
        if (planet) {
          sessionState = planet.state;
          break;
        }
      }
  ...
  const playLaserSound = useCallback(() => {
    ...
    const gain = ctx.createGain();
    gain.gain.setValueAtTime(0.3 * cfg.masterVolume, now);
    ...
    osc.start(now);
    osc.stop(now + 0.2);
    setTimeout(() => { osc.disconnect(); gain.disconnect(); }, 300);
  }, []);

  const handleShoot = useCallback(() => {
    setUniforms(prev => ({ ...prev, u_fireTime: accumulatedTimeRef.current }));
  ...
  const pressKey = useCallback((key: string) => {
    ...
    // Role Play Keybinds
    if (lowerKey === 'l') landShip();
    if (lowerKey === 't') takeOffShip();
    if (lowerKey === 'x') exitShip();
    if (lowerKey === 'e') enterShip();
    if (lowerKey === 'u') tryEscape();
    if (lowerKey === 'h') toggleHide();
    if (lowerKey === 'r') refuel();
    if (lowerKey === 'c') capturePrisoner();
    if (lowerKey === 'v') releasePrisoner();
    if (lowerKey === 'k') executePrisoner();

    keysPressed.current.add(lowerKey);
    setPressedKeys(prev => new Set(prev).add(lowerKey));
  }, [initAudio, handleShoot]);
  ...
  const setPlayerRole = useCallback((role: PlayerRole) => {
    setRolePlayState(prev => ({ ...prev, role }));
  }, []);

  const landShip = useCallback(() => {
    if (cameraRef.current.position[1] < 15) {
      setRolePlayState(prev => ({ ...prev, isLanded: true }));
      cameraRef.current.position[1] = 2.0;
      cameraVelocityRef.current = [0, 0, 0];
    }
  }, []);

  const takeOffShip = useCallback(() => {
    if (rolePlayStateRef.current.fuel > 0) {
      setRolePlayState(prev => ({ ...prev, isLanded: false }));
    }
  }, []);

  const exitShip = useCallback(() => {
    if (rolePlayStateRef.current.isLanded) {
      setRolePlayState(prev => ({ ...prev, isInsideShip: false }));
    }
  }, []);

  const enterShip = useCallback(() => {
    setRolePlayState(prev => ({ ...prev, isInsideShip: true }));
  }, []);

  const tryEscape = useCallback(() => {
    if (rolePlayStateRef.current.role === 'prisoner' && rolePlayStateRef.current.prisonerStatus === 'imprisoned') {
      if (Math.random() > 0.7) {
        setRolePlayState(prev => ({ ...prev, prisonerStatus: 'escaped', isDetected: false }));
      }
    }
  }, []);

  const toggleHide = useCallback(() => {
    if (rolePlayStateRef.current.role === 'prisoner' && rolePlayStateRef.current.prisonerStatus === 'escaped') {
      setRolePlayState(prev => ({ 
        ...prev, 
        prisonerStatus: prev.prisonerStatus === 'hidden' ? 'escaped' : 'hidden',
        isDetected: false 
      }));
    }
  }, []);
  ...
  const refuel = useCallback(() => {
    if (rolePlayStateRef.current.isLanded) {
      setRolePlayState(prev => ({ ...prev, fuel: 100 }));
    }
  }, []);

  const attemptAdminLogin = useCallback((password: string) => {
    const adminPassword = import.meta.env.VITE_GRAND_ADMIN_TEAM_PASSWORD;
    // Use the environment variable if set, otherwise fallback to the previous password for now
    const targetPassword = adminPassword || 'GrimCallings12345687337';
    ...
    if (password === targetPassword) {
      setRolePlayState(prev => ({
        ...
        isAdmin: true,
        isGrandAdmin: true,
        hasMasterPlanAccess: true,
        employeeRank: 1, // Highest rank
      }));
      return true;
    }
    return false;
  ...
  const capturePrisoner = useCallback(() => {
    if (rolePlayStateRef.current.role === 'pilot' || rolePlayStateRef.current.role === 'passenger') {
      setRolePlayState(prev => ({ ...prev, prisonerStatus: 'imprisoned', isDetected: false }));
    }
  }, []);

  const releasePrisoner = useCallback(() => {
    if (rolePlayStateRef.current.role === 'pilot') {
      setRolePlayState(prev => ({ ...prev, prisonerStatus: 'escaped', isDetected: false }));
    }
  }, []);

  const executePrisoner = useCallback(() => {
    if (rolePlayStateRef.current.role === 'pilot') {
      setRolePlayState(prev => ({ ...prev, prisonerStatus: 'dead', isDetected: false }));
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const gameLoop = (timestamp: number) => {
        ...
        // --- Role Play Logic ---
        const rp = rolePlayStateRef.current;
        
        // Fuel depletion
        if (!rp.isLanded && rp.isInsideShip && rp.role === 'pilot') {
            const fuelBurn = (Math.abs(fwd) + Math.abs(str) + Math.abs(asc) * 2 + 0.1) * 0.05 * dt;
            const newFuel = Math.max(0, rp.fuel - fuelBurn);
            if (newFuel !== rp.fuel) {
                setRolePlayState(prev => ({ ...prev, fuel: newFuel }));
            }
        }

        // Detection logic
        if (rp.prisonerStatus === 'escaped' && !rp.isDetected) {
            if (Math.random() < 0.001) {
                setRolePlayState(prev => ({ ...prev, isDetected: true }));
            }
        }

        // Movement constraints
        if (rp.isLanded || !rp.isInsideShip || rp.fuel <= 0 || rp.role !== 'pilot') {
            fwd = 0; str = 0; asc = 0;
  ...
  }, [playGenerativeNote, playArpNote, playRhythm, playCollisionSound, playLaserSound]);
  ...
  return {
    ...
    rolePlayState, setPlayerRole, landShip, takeOffShip, exitShip, enterShip, tryEscape, toggleHide, refuel, capturePrisoner, releasePrisoner, executePrisoner, attemptAdminLogin,
    user, userProfile, handleLogin, handleLogout
  };
};
...
export const useAppStoreComplete = (): AppContextType => {
    ...
    const { handleTerraformPress, handleTerraformRelease, terraformPower, isMoving, debugElevation, debugArpVolume, debugCameraAltitude, debugCameraPitch, debugCameraDistance, collisionState, collisionProximity, collisionThresholdRed, setCollisionThresholdRed, collisionThresholdYellow, setCollisionThresholdYellow, isInteracting, setIsInteracting, viewMode, setViewMode, viewModeTransition, shipConfig, effectiveShipConfigRef, handleShipConfigChange, addShipModulation, updateShipModulation, removeShipModulation, debugCollisionPointRef, debugRayStartPointRef, debugRayEndPointRef, debugCollisionDistanceRef, getSessionStateJson, rolePlayState, setPlayerRole, landShip, takeOffShip, exitShip, enterShip, tryEscape, toggleHide, refuel, capturePrisoner, releasePrisoner, executePrisoner, attemptAdminLogin, user, userProfile, handleLogin, handleLogout, ...rest } = pilot;
    return { ...dummy, ...rest, handleTerraformPress, handleTerraformRelease, terraformPower, isMoving, debugElevation, debugArpVolume, debugCameraAltitude, debugCameraPitch, debugCameraDistance, collisionState, collisionProximity, collisionThresholdRed, setCollisionThresholdRed, collisionThresholdYellow, setCollisionThresholdYellow, isInteracting, setIsInteracting, viewMode, setViewMode, viewModeTransition, shipConfig, effectiveShipConfigRef, handleShipConfigChange, addShipModulation, updateShipModulation, removeShipModulation, debugCollisionPointRef, debugRayStartPointRef, debugRayEndPointRef, debugCollisionDistanceRef, getSessionStateJson, rolePlayState, setPlayerRole, landShip, takeOffShip, exitShip, enterShip, tryEscape, toggleHide, refuel, capturePrisoner, releasePrisoner, executePrisoner, attemptAdminLogin, user, userProfile, handleLogin, handleLogout } as AppContextType;
}
