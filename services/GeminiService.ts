...
import { GoogleGenAI, Type, Modality, ThinkingLevel, GenerateContentResponse } from "@google/genai";
...
const PRO_IMAGE_MODEL = 'gemini-3.1-flash-image-preview';
const STUDIO_IMAGE_MODEL = 'gemini-3-pro-image-preview';
...
export const chatWithGemini = async (
    messages: { role: 'user' | 'model', content: string, media?: { data: string, mimeType: string }[] }[], 
    systemInstruction: string, 
    useGrounding: boolean = false,
    useHighThinking: boolean = false
) => {
    const chat = ai.chats.create({
        model: useHighThinking ? PRO_MODEL : DEFAULT_MODEL,
        config: {
            systemInstruction,
            tools: useGrounding ? [{ googleSearch: {} }, { googleMaps: {} }] : undefined,
...
export const generateImage = async (prompt: string, config: { aspectRatio?: string, imageSize?: string, quality?: 'standard' | 'high' | 'studio' } = {}) => {
    let selectedModel = IMAGE_MODEL;
    if (config.quality === 'high') selectedModel = PRO_IMAGE_MODEL;
    if (config.quality === 'studio') selectedModel = STUDIO_IMAGE_MODEL;
...
export const analyzeMedia = async (prompt: string, media: { data: string, mimeType: string }[]) => {
    const response = await ai.models.generateContent({
        ...
        contents: {
            parts: [
                ...media.map(m => ({ inlineData: { data: m.data.split(',')[1], mimeType: m.mimeType } })),
                { text: prompt }
            ]
        }
    });
...
export const lowLatencyResponse = async (prompt: string) => {
    const response = await ai.models.generateContent({
        model: LITE_MODEL,
        contents: prompt
    });
...
export const connectLiveAPI = (callbacks: any) => {
    return ai.live.connect({
        ...
        config: {
            ...
            systemInstruction: "You are the Nexus-Verse AI assistant. You are helpful, slightly mysterious, and deeply integrated into the ship's systems.",
        },
    });
};
