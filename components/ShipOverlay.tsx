...
void main() {
    ...
    if(t < MAX_DIST) {
        ...
        // --- Muzzle Flash ---
        float fire = u_time - u_fireTime;
        if (fire > 0.0 && fire < 0.2) {
            // Flash at the front of the ship (negative Z)
            float muzzleMask = smoothstep(-0.5, -1.5, localP.z) * (1.0 - smoothstep(0.0, 0.5, abs(localP.x)));
            float flash = smoothstep(0.2, 0.0, fire) * 5.0;
            col += vec3(0.0, 1.0, 1.0) * muzzleMask * flash;
        }
...
export const ShipOverlay: React.FC = () => {
    const { viewMode, pressedKeys, controlConfig, effectiveShipConfigRef, cameraAngularVelocityRef, shipConfig, uniforms } = useAppContext();
    ...
    useEffect(() => {
        ...
        const locs = {
            ...
            uFireTime: gl.getUniformLocation(p, 'u_fireTime'),
        };
        ...
        const render = (t: number) => {
            ...
            gl.uniform1f(locs.uFireTime, uniformsRef.current.u_fireTime ?? 0.0);
