...
import { XCircleIcon, TerraformIcon, AdjustmentsIcon, SparklesIcon, ShieldCheckIcon } from './Icons';
...
const RolePlayPanel: React.FC = () => {
    const { rolePlayState, setPlayerRole, landShip, takeOffShip, exitShip, enterShip, tryEscape, toggleHide, refuel, capturePrisoner, releasePrisoner, executePrisoner, attemptAdminLogin } = useAppContext();
    const { role, isInsideShip, isLanded, prisonerStatus, fuel, isDetected, isAdmin, hasMasterPlanAccess } = rolePlayState;
    ...
    return (
        <div className="space-y-6">
            <div className="p-4 bg-gray-900/50 rounded-lg border border-gray-700 space-y-4">
                <h3 className="text-sm font-semibold text-cyan-300 uppercase tracking-wider">Character Role</h3>
                <div className="grid grid-cols-3 gap-2">
                    {(['pilot', 'passenger', 'prisoner'] as const).map(r => (
                        <button
                            key={r}
            ...
            <div className="p-4 bg-gray-900/50 rounded-lg border border-gray-700 space-y-4">
                <h3 className="text-sm font-semibold text-cyan-300 uppercase tracking-wider">Nexus Authority</h3>
                
                <div className="grid grid-cols-1 gap-3">
                    <button
                        onClick={() => setIsManifestoOpen(true)}
                        className="w-full py-3 bg-cyan-600/10 hover:bg-cyan-600/20 border border-cyan-500/50 text-cyan-400 text-xs font-bold rounded transition-all flex items-center justify-center gap-2"
                    >
                        <ShieldCheckIcon className="w-4 h-4" />
                        READ THE MASTER MANIFESTO
                    </button>
                    ...
                    {!isAdmin ? (
                        <div className="flex gap-2">
                            <input
                                ...
                                className="flex-grow p-2 bg-gray-800 border border-gray-700 rounded text-xs text-white focus:outline-none focus:border-cyan-500"
                                onKeyDown={(e) => e.key === 'Enter' && handleLogin()}
                            />
                            <button
                                onClick={handleLogin}
                                className="px-3 py-2 bg-cyan-600 hover:bg-cyan-500 text-white text-xs font-bold rounded transition-colors"
                            >
                                LOGIN
                            </button>
                        </div>
                        ...
                        <div className="space-y-3">
                            <div className="flex items-center justify-between">
                                <span className="text-xs text-green-400 font-bold uppercase">Authenticated: Grand Admin</span>
                                <span className="text-[10px] text-white/40 font-mono">RANK: 01</span>
                            </div>
                            {hasMasterPlanAccess && (
                                <button
                                    onClick={() => setIsMasterPlanOpen(true)}
                                    className="w-full py-3 bg-red-600/20 hover:bg-red-600/30 border border-red-500 text-red-400 text-xs font-bold rounded transition-all shadow-[0_0_15px_rgba(239,68,68,0.2)]"
                                >
                                    VIEW THE GRAND MASTER PLAN
                                </button>
                            )}
                        </div>
                    )}
            ...
            <MasterPlanOverlay isOpen={isMasterPlanOpen} onClose={() => setIsMasterPlanOpen(false)} />
            <ManifestoOverlay isOpen={isManifestoOpen} onClose={() => setIsManifestoOpen(false)} />

            <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-gray-900/50 rounded-lg border border-gray-700 space-y-3">
                    <h3 className="text-sm font-semibold text-cyan-300 uppercase tracking-wider">Ship Status</h3>
                    <div className="space-y-2">
                        <div className="flex justify-between text-xs">
                            <span className="text-gray-400">Position:</span>
                            <span className="text-white">{isInsideShip ? 'Inside' : 'Outside'}</span>
                        </div>
                        <div className="flex justify-between text-xs">
                            <span className="text-gray-400">Landing:</span>
                            <span className={isLanded ? 'text-green-400' : 'text-blue-400'}>{isLanded ? 'Landed' : 'In Flight'}</span>
                        </div>
                        <div className="flex justify-between text-xs">
                            <span className="text-gray-400">Fuel:</span>
                            <span className={fuel < 20 ? 'text-red-400' : 'text-white'}>{Math.round(fuel)}%</span>
                        </div>
                    </div>
                </div>

                <div className="p-4 bg-gray-900/50 rounded-lg border border-gray-700 space-y-3">
                    <h3 className="text-sm font-semibold text-cyan-300 uppercase tracking-wider">Actions</h3>
                    <div className="grid grid-cols-1 gap-2">
                        {!isLanded ? (
                            <button onClick={landShip} className="text-[10px] bg-gray-700 hover:bg-gray-600 p-1.5 rounded text-white">LAND SHIP [L]</button>
                        ) : (
                            <button onClick={takeOffShip} className="text-[10px] bg-gray-700 hover:bg-gray-600 p-1.5 rounded text-white">TAKE OFF [T]</button>
                        )}
                        {isLanded && (
                            <button onClick={isInsideShip ? exitShip : enterShip} className="text-[10px] bg-gray-700 hover:bg-gray-600 p-1.5 rounded text-white">
                                {isInsideShip ? 'EXIT SHIP [X]' : 'ENTER SHIP [E]'}
                            </button>
                        )}
                        {isLanded && <button onClick={refuel} className="text-[10px] bg-green-900/40 hover:bg-green-800/60 p-1.5 rounded text-green-400 border border-green-500/30">REFUEL [R]</button>}
                    </div>
                </div>
            </div>

            {role === 'prisoner' && (
                <div className="p-4 bg-red-900/10 rounded-lg border border-red-900/30 space-y-4">
                    <h3 className="text-sm font-semibold text-red-400 uppercase tracking-wider">Prisoner Mechanics</h3>
                    <div className="flex justify-between items-center text-xs">
                        <span className="text-gray-400">Current Status:</span>
                        <span className="font-bold text-red-400">{prisonerStatus.toUpperCase()}</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                        <button 
                            onClick={tryEscape} 
                            disabled={prisonerStatus !== 'imprisoned'}
                            className="text-[10px] bg-red-900/40 hover:bg-red-800/60 p-2 rounded text-red-400 border border-red-500/30 disabled:opacity-30"
                        >
                            TRY ESCAPE [U]
                        </button>
                        <button 
                            onClick={toggleHide} 
                            disabled={prisonerStatus === 'imprisoned' || prisonerStatus === 'dead'}
                            className="text-[10px] bg-yellow-900/40 hover:bg-yellow-800/60 p-2 rounded text-yellow-400 border border-yellow-500/30 disabled:opacity-30"
                        >
                            {prisonerStatus === 'hidden' ? 'REVEAL SELF [H]' : 'HIDE [H]'}
                        </button>
                    </div>
                    {isDetected && (
                        <div className="p-2 bg-red-600/20 border border-red-600 text-red-500 text-center text-[10px] font-bold animate-pulse rounded">
            ...
            {role === 'pilot' && (
                <div className="p-4 bg-blue-900/10 rounded-lg border border-blue-900/30 space-y-4">
                    <h3 className="text-sm font-semibold text-blue-400 uppercase tracking-wider">Pilot Authority</h3>
                    <div className="grid grid-cols-3 gap-2">
                        <button onClick={capturePrisoner} className="text-[10px] bg-blue-900/40 hover:bg-blue-800/60 p-2 rounded text-blue-400 border border-blue-500/30">CAPTURE [C]</button>
                        <button onClick={releasePrisoner} className="text-[10px] bg-green-900/40 hover:bg-green-800/60 p-2 rounded text-green-400 border border-green-500/30">RELEASE [V]</button>
                        <button onClick={executePrisoner} className="text-[10px] bg-red-900/40 hover:bg-red-800/60 p-2 rounded text-red-400 border border-red-500/30">EXECUTE [K]</button>
                    </div>
                </div>
            )}
        </div>
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="space-y-4 border-b border-gray-700 pb-6">
                <h3 className="text-sm font-semibold text-gray-400 px-1">Host Identity</h3>
                {!user ? (
                    <div className="p-4 bg-gray-900/50 rounded-lg border border-gray-700 flex flex-col items-center gap-4">
                        <p className="text-xs text-gray-400 text-center">No host identity detected. Undergo the Ritual of Binding to persist your neural patterns.</p>
                        <button 
                            onClick={handleLogin}
                            className="w-full py-2 bg-cyan-600 hover:bg-cyan-500 text-white font-bold rounded-md transition-colors text-sm"
                        >
                            Sign In with Google
                        </button>
                    </div>
                    ...
                    <div className="p-4 bg-gray-900/50 rounded-lg border border-gray-700 flex items-center gap-4">
                        ...
                        <div className="flex-grow">
                            ...
                            <p className="text-[10px] text-cyan-400 uppercase tracking-widest">{userProfile?.role || 'Passenger'}</p>
                            <p className="text-[10px] text-gray-500">{user.email}</p>
                        </div>
                        <button 
                            onClick={handleLogout}
                            className="px-3 py-1.5 bg-gray-800 hover:bg-red-900/40 text-gray-400 hover:text-red-400 border border-gray-700 hover:border-red-900/50 rounded text-[10px] font-bold transition-all"
                        >
                            LOGOUT
                        </button>
                    </div>
...
const IntelligencePanel: React.FC = () => {
    const [subTab, setSubTab] = useState<'chat' | 'assets' | 'live'>('chat');
    ...
    if (!user) {
        return (
            <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <h3 className="text-xl font-bold text-white mb-2">Nexus Intelligence Access Restricted</h3>
                <p className="text-gray-400 max-w-md mb-8">
                    To access the advanced neural networks and generative capabilities of the Nexus-Verse, you must first undergo the Ritual of Binding.
                </p>
                <button 
                    onClick={handleLogin}
                    className="px-8 py-3 bg-cyan-600 hover:bg-cyan-500 text-white font-bold rounded-full transition-all transform hover:scale-105 shadow-lg shadow-cyan-900/20"
                >
                    Initiate Ritual (Sign In)
                </button>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-center gap-2 p-1 bg-black/40 rounded-lg border border-gray-800 w-fit mx-auto mb-8">
                <button 
                    onClick={() => setSubTab('chat')}
                    className={`px-4 py-1.5 rounded-md text-xs font-bold transition-all ${subTab === 'chat' ? 'bg-cyan-600 text-white shadow-lg shadow-cyan-900/40' : 'text-gray-500 hover:text-gray-300'}`}
                >
                    NEURAL CHAT
                </button>
                <button 
                    onClick={() => setSubTab('assets')}
                    className={`px-4 py-1.5 rounded-md text-xs font-bold transition-all ${subTab === 'assets' ? 'bg-cyan-600 text-white shadow-lg shadow-cyan-900/40' : 'text-gray-500 hover:text-gray-300'}`}
                >
                    ASSET FORGE
                </button>
                <button 
                    onClick={() => setSubTab('live')}
                    className={`px-4 py-1.5 rounded-md text-xs font-bold transition-all ${subTab === 'live' ? 'bg-cyan-600 text-white shadow-lg shadow-cyan-900/40' : 'text-gray-500 hover:text-gray-300'}`}
                >
                    LIVE LINK
                </button>
            </div>

            <div className="bg-gray-900/30 rounded-xl border border-gray-800/50 overflow-hidden">
                {subTab === 'chat' && <ChatInterface />}
                {subTab === 'assets' && <AssetGenerator />}
                {subTab === 'live' && <LiveAudioInterface />}
            </div>
        </div>
    );
};
...
export const ControlsPanel: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'sliders' | 'terraform' | 'controls' | 'settings' | 'sound' | 'collision' | 'ship' | 'roleplay' | 'intelligence' | 'navigation'>('sliders');
  ...
  return (
    <>
      ...
      <div
        className="fixed inset-0 z-50 bg-gray-900/90 backdrop-blur-xl flex flex-col overflow-hidden"
        role="dialog"
        aria-modal="true"
      ...
      >
        {/* Header */}
        <div className="flex-shrink-0 flex items-center justify-between px-6 py-6 border-b border-gray-700/50">
          <div className="flex flex-col">
            <h2 id="controls-heading" className="text-2xl font-bold text-white flex items-center gap-3">
                <AdjustmentsIcon className="w-8 h-8 text-cyan-400" />
                <span className="tracking-tight">GEMINI SHADER PILOT</span>
                <span className="text-xs font-mono text-cyan-500/50 bg-cyan-500/10 px-2 py-0.5 rounded border border-cyan-500/20 ml-2">
                    V2.5.0
                </span>
            </h2>
            <p className="text-xs text-gray-500 mt-1 ml-11">
                Advanced Neural Fractal Navigation System • by <a href="https://x.com/pitaru" target="_blank" rel="noopener noreferrer" className="text-cyan-400/60 hover:text-cyan-400 transition-colors">@pitaru</a>
            </p>
          </div>
          ...
          <div className="flex items-center gap-4">
            <button 
                onClick={() => setIsControlsOpen(false)}
                className="group flex items-center gap-2 text-gray-400 hover:text-white transition-all bg-gray-800/50 hover:bg-gray-700/50 px-4 py-2 rounded-full border border-gray-700"
                aria-label="Close controls"
            >
                <span className="text-xs font-bold tracking-widest uppercase">Close Interface</span>
                <XCircleIcon className="w-6 h-6 group-hover:rotate-90 transition-transform duration-300" />
            </button>
          </div>
        </div>

        <div className="flex-shrink-0 flex justify-center overflow-x-auto px-6 py-4 gap-2 border-b border-gray-700/30 bg-black/20 no-scrollbar">
            <TabButton label="World" isActive={activeTab === 'sliders'} onClick={() => setActiveTab('sliders')} />
            <TabButton label="Navigation" isActive={activeTab === 'navigation'} onClick={() => setActiveTab('navigation')} />
            ...
            <TabButton label="Role Play" isActive={activeTab === 'roleplay'} onClick={() => setActiveTab('roleplay')} />
            
            {EDITMODE && (
                <>
                    <div className="w-px h-8 bg-gray-700/50 mx-2 self-center" />
        ...
        <div className="flex-grow overflow-y-auto custom-scrollbar">
            <div className="max-w-5xl mx-auto p-8 lg:p-12">
                ...
                {activeTab === 'roleplay' && <RolePlayPanel />}
                ...
                {activeTab === 'navigation' && <WorldSelector />}
            </div>
        </div>
        
         {/* Footer */}
        {sourceAuthor && activeTab === 'sliders' && (
            <div className="flex-shrink-0 px-6 py-4 border-t border-gray-700/30 bg-black/40 text-center">
                <a 
                    href={sessionSource!} 
                    target="_blank" 
                    rel="noopener noreferrer" 
                    className="text-xs text-cyan-400/60 hover:text-cyan-400 transition-colors flex items-center justify-center gap-2"
                >
