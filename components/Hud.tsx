...
export const Hud: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const render = () => {
            ...
            // Crosshair stays visible even in chase mode (hudAlpha might be 0)
            const crosshairAlpha = Math.max(0.3, hudAlpha);
            ctx.strokeStyle = `rgba(255, 255, 255, ${0.3 * crosshairAlpha})`;
            ctx.beginPath();
            ctx.moveTo(cx - 20, cy); ctx.lineTo(cx - 5, cy);
            ctx.moveTo(cx + 5, cy); ctx.lineTo(cx + 20, cy);
            ctx.moveTo(cx, cy - 15); ctx.lineTo(cx, cy - 5);
            ctx.stroke();
            ctx.fillStyle = `rgba(255,255,255,${0.5 * crosshairAlpha})`;
